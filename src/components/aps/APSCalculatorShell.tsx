...
import { Plus, Trash2, Save, Check, BookOpen, AlertCircle, ChevronRight } from 'lucide-react';
...
const tierStyles: Record<string, string> = {
  bachelor:     'text-ugreen-700 bg-ugreen-50 border-ugreen-200',
  diploma:      'text-ugold-700 bg-ugold-50 border-ugold-200',
  'higher-cert':'text-navy-700 bg-navy-50 border-navy-200',
  none:         'text-ured-700 bg-ured-50 border-ured-200',
};

function LevelBadge({ level, mark }: { level: number; mark: number }) {
  const color =
    mark >= 50 ? 'bg-ugreen-100 text-ugreen-700'
    : mark >= 40 ? 'bg-ugold-100 text-ugold-700'
...
export function APSCalculatorShell({ onResult }: { onResult?: (r: APSResult, subjects: Subject[]) => void }) {
  ...
  const nearCourses = CAREER_DATABASE.filter(c => result && result.standardAps < c.minAps && result.standardAps >= c.minAps - 4);

  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 animate-fade-up">
      {/* Subject inputs — wider column */}
      <div className="lg:col-span-3 card p-6 space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {(['science', 'commerce', 'humanities'] as const).map(stream => (
              <button
                ...
                className="px-2.5 py-1 rounded-md border border-navy-200 text-xs font-semibold text-navy-600 hover:bg-navy-50 transition-colors capitalize"
              >
                {stream}
              </button>
            ))}
              ...
              <div key={i} className="flex items-center gap-2 bg-slate-50 dark:bg-navy-800 rounded-lg px-3 py-2.5 border border-navy-100 dark:border-navy-700">
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    className="w-12 text-center bg-transparent border-b border-dashed border-navy-400 text-sm font-bold text-navy-900 dark:text-white outline-none"
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 border border-dashed border-navy-300 rounded-lg py-2 text-sm font-semibold text-navy-500 hover:border-navy-500 hover:text-navy-700 disabled:opacity-40 transition-colors"
          >
      ...
      <div className="lg:col-span-2 flex flex-col gap-4">
        ...
        <div className="card p-6 flex flex-col items-center text-center gap-3">
          <div className="relative w-32 h-32">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
              ...
              <circle
                ...
                strokeLinecap="round"
                className="text-navy-800 transition-all duration-700"
              />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-4xl font-black text-navy-900 dark:text-white leading-none">{result?.standardAps ?? 0}</span>
          ...
          {result && (
            <div className={cn('w-full rounded-xl border px-4 py-3 text-sm', tierStyles[result.qualificationTier])}>
        ...
        <div className="card p-5 space-y-3">
          ...
          {qualifiedCourses.length === 0 ? (
            <div className="flex items-start gap-2 text-xs text-navy-400">
              ...
              No courses at current APS. Improve any 2 subjects to unlock diploma pathways.
            </div>
          ) : (
            <ul className="space-y-1.5">
              {qualifiedCourses.map(c => (
                <li key={c.id} className="flex items-center justify-between gap-2 text-xs">
                  ...
                  <span className={cn('badge text-[10px]', c.demandMpumalanga === 'CRITICAL' ? 'badge-red' : 'badge-green')}>
                    {c.demandMpumalanga}
                  </span>
                </li>
              ))}
          ...
          {nearCourses.length > 0 && (
            <>
              <h4 className="font-bold text-navy-700 dark:text-navy-300 text-xs pt-2 border-t border-navy-100 dark:border-navy-800">Close (within 4 APS)</h4>
              <ul className="space-y-1.5">
                {nearCourses.map(c => (
                  <li key={c.id} className="flex items-center gap-2 text-xs text-navy-400">
                    <ChevronRight size={11} className="flex-shrink-0" />
                    <span>{c.title}</span>
                    <span className="ml-auto font-semibold text-ugold-600">needs {c.minAps - (result?.standardAps ?? 0)} more pts</span>
                  </li>
                ))}
              </ul>
            </>
