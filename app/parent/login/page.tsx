      ...
      <div className="w-full max-w-md rounded-2xl border border-line bg-paper-raised p-6 sm:p-8 shadow-md">
        <div className="flex flex-col gap-2 text-center mb-6">
          ...
          <p className="text-xs text-ink-soft">
            POPIA-compliant portal for minor learner consent, deadline alerts, and university funding monitoring.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          ...
          >
            {submitting ? "Signing In..." : "Sign In to Parent Portal"}
          </button>
        </form>

        <div className="mt-6 border-t border-line/60 pt-4 text-center text-xs text-ink-soft">
          Don&apos;t have an account yet? <Link href="/account" className="font-bold text-brand-teal hover:underline">Create Parent Account</Link>
        </div>
      </div>
    </main>
  );
